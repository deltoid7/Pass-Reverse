...
class CTCLayer(layers.Layer):
    ...
    def call(self, y_true, y_pred):
        ...
        # Try tf.keras.backend.ctc_batch_cost first (TensorFlow 2.x compatible)
        # If not available, fall back to tf.nn.ctc_loss
        try:
            loss = tf.keras.backend.ctc_batch_cost(y_true, y_pred, input_length, label_length)
        except (AttributeError, TypeError):
            # Fallback: use tf.nn.ctc_loss
            # Convert dense labels to sparse format using vectorized operations
            y_true_int = tf.cast(y_true, dtype="int32")
            batch_size = tf.shape(y_true)[0]
            max_label_len = tf.shape(y_true)[1]
            
            # Create a mask for valid label positions
            batch_idx = tf.range(batch_size, dtype="int64")
            label_idx = tf.range(max_label_len, dtype="int64")
            
            # Create meshgrid for all positions (TensorFlow compatible)
            batch_grid = tf.tile(tf.expand_dims(batch_idx, 1), [1, max_label_len])
            label_grid = tf.tile(tf.expand_dims(label_idx, 0), [batch_size, 1])
            
            # Create mask: label_grid < label_length for each batch
            label_length_flat = tf.reshape(label_length, [-1])
            valid_mask = tf.less(label_grid, tf.expand_dims(label_length_flat, 1))
            
            # Flatten and get valid indices and values
            batch_flat = tf.reshape(batch_grid, [-1])
            label_flat = tf.reshape(label_grid, [-1])
            values_flat = tf.reshape(y_true_int, [-1])
            valid_flat = tf.reshape(valid_mask, [-1])
            
            # Filter valid positions
            valid_batch = tf.boolean_mask(batch_flat, valid_flat)
            valid_label = tf.boolean_mask(label_flat, valid_flat)
            valid_values = tf.boolean_mask(values_flat, valid_flat)
            
            # Create indices tensor
            indices = tf.stack([valid_batch, valid_label], axis=1)
            ...
            loss = tf.nn.ctc_loss(
                ...
                logit_length=tf.reshape(input_length, [-1]),
...
class ApplyModel:
    ...
    def __init__(self, 
        ...
        # Get input and output layers for prediction model
        # In Keras 3.x, use model.inputs[0] for the first input instead of get_layer().input
        input_layer = self.model.inputs[0]  # First input is the image
        output_layer = self.model.get_layer(name="dense2").output
        self.prediction_model = keras.models.Model(inputs=input_layer, outputs=output_layer)
    ...
    def decode_batch_predictions(self, pred):
        ...
        # For Keras 3.x, use tf.nn.ctc_greedy_decoder or tf.keras.backend.ctc_decode
        try:
            # Try using tf.keras.backend.ctc_decode (may work in some versions)
            results = tf.keras.backend.ctc_decode(pred, input_length=input_len, greedy=True)[0][0][
                :, :self.max_length
            ]
        except AttributeError:
            # Fallback to manual decoding using argmax
            pred = tf.nn.softmax(pred, axis=-1)
            results = tf.argmax(pred, axis=-1)[:, :self.max_length]
